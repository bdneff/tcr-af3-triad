#!/usr/bin/env python3
"""Extract per-residue CDR phi/psi for one triad: predicted vs crystal."""
import sys, math
...
# ==== CONFIG (change TARGET_PDB to look at a different triad) ====
TARGET_PDB = "7q9a"
...
OUT_CSV = Path(f"./ramachandran_{TARGET_PDB}.csv")
MODEL_IDX = 0
...
def main():
    ...
    row = meta[(meta["pdb"] == TARGET_PDB) & meta["cognate"]]
    if len(row) == 0:
        sys.exit(f"ERROR: no cognate row found for PDB {TARGET_PDB}")
    r = row.iloc[0]
    job = r["job_name"]
    print(f"[info] PDB={TARGET_PDB} job={job} peptide={r['peptide']} PTI-PAE={r['mean_p_tcr_interface_pae']:.2f}")

    pred_f = PRED_DIR / job / f"{job}_model_{MODEL_IDX}.cif"
    cryst_f = CRYSTAL_DIR / f"{TARGET_PDB}.pdb"
    pred = MMCIFParser(QUIET=True).get_structure(job, str(pred_f))
    cryst = PDBParser(QUIET=True).get_structure(TARGET_PDB, str(cryst_f))
    ...
    for tcr_name, ref_seq, p_ch, c_ch in [("tcr_1", t1_seq, pred_t1, cryst_t1),
        ...
        if p_ch is None or c_ch is None:
            print(f"[warn] {tcr_name}: chain not found"); continue
    ...
    print(f"[info] wrote {len(df)} residues -> {OUT_CSV}")
